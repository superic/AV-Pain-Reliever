...
Smaller changes riding along: `CameraDiscovery` becomes public and `VirtualCameraActivator.hostCanSeeVirtualCamera` uses it instead of a hand-duplicated device-type list (the duplication predated this work and had already drifted into two copies of the same four device types); the 8pt status dot is extracted as `StatusDot` so the extension-state row and the feed row can't drift; the Settings window grows 380 → 460 tall so the preview card sits above the fold rather than one scroll down.
