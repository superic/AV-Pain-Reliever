...
- **One relaunch, latched across the process boundary.** `VirtualCameraRelaunchGuard` writes a marker to `UserDefaults` *before* quitting, so the process that comes back up knows it already spent its retry. If that process's own poll + re-check also fail, the escalation declines and the manual recovery advice stands — there is no quit/launch loop by construction. The marker clears the moment any process confirms it can see the camera, so a later unrelated episode gets its own single attempt. Reads never write the default, per the `SettingsStore` lazy-default rule. Six unit tests cover the latch, including the loop guarantee and the re-arm.
